...
export function clusterAddresses(
  ...
  for (const tx of txs) {
    for (const o of tx.outputs) {
      if (labels.get(o.address)?.type !== 'deposit') continue
      const set = depositSenders.get(o.address) ?? new Set()
      for (const i of tx.inputs) if (i.address && i.address !== o.address) set.add(i.address)
  ...
  for (const members of groups.values()) {
    ...
    const labelled = members.map(m => labels.get(m)).find(l => l && l.type !== 'deposit')
    const c: Cluster = {
      id: id++,
